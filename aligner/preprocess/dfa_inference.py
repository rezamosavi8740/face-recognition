...
def dfa_warp(input_data, landmark, input_size =160, output_size =112):
    ...
    cv2_tfms = aligner_helper.get_cv2_affine_from_landmark(torch.from_numpy(landmark.reshape(-1, 10)), reference_ldmk, input_size, input_size)
    ...
    aligned_x = F.grid_sample(torch.from_numpy(input_data) + 1, grid, align_corners=True) - 1  # +1, -1 for making padding pixel 0
